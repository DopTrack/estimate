...
if len(selected_files) == 0:
    raise RuntimeError(
        f"No matching data files found for satellite={satellite_name}, "
        f"start_time={start_time}, estimate_days={estimate_days}"
    )
...
with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
    ...
    for result in valid_validation_results:
        ...
        passes_start_times = result["passes_start_times"]
        passes_end_times = result["passes_end_times"]

        for i in range(number_of_passes):

            bias_epoch = passes_start_times[i] if i < len(passes_start_times) else np.nan
            bias_end_epoch = passes_end_times[i] if i < len(passes_end_times) else np.nan
            ...
            bias_rows.append({
                ...
                "Bias_Epoch_s": bias_epoch,
                "Bias_End_Epoch_s": bias_end_epoch,

                "Bias_Time_since_chunk_start_days": (
                    (bias_epoch - passes_start_times[0]) / constants.JULIAN_DAY
                    if len(passes_start_times) > 0 and np.isfinite(bias_epoch)
                    else np.nan
                ),

                "Bias_Time_since_chunk_start_hours": (
                    (bias_epoch - passes_start_times[0]) / 3600.0
                    if len(passes_start_times) > 0 and np.isfinite(bias_epoch)
                    else np.nan
                ),

                "Absolute_Constant_Bias": constant_biases[i] if i < len(constant_biases) else np.nan,
                "Linear_Constant_Bias": linear_biases[i] if i < len(linear_biases) else np.nan
            })
        ...
        for i, value in enumerate(drag_estimates):
            ...
            drag_epoch = drag_start_times[i] if i < len(drag_start_times) else np.nan
            ...
            drag_rows.append({
                ...
                "Drag_Epoch_s": drag_epoch,

                "Drag_Time_since_chunk_start_days": (
                    (drag_epoch - drag_start_times[0]) / constants.JULIAN_DAY
                    if len(drag_start_times) > 0 and np.isfinite(drag_epoch)
                    else np.nan
                ),

                "Drag_Time_since_chunk_start_hours": (
                    (drag_epoch - drag_start_times[0]) / 3600.0
                    if len(drag_start_times) > 0 and np.isfinite(drag_epoch)
                    else np.nan
                ),
    ...
    # --------------------------------------------------------
    # Plot drag coefficient estimates over time
    # --------------------------------------------------------

    if len(df_drag) > 0:

        df_drag_plot = df_drag.copy()

        df_drag_plot = df_drag_plot.dropna(
            subset=["Drag_Epoch_s", "Estimated_Cd"]
        )

        df_drag_plot = df_drag_plot.sort_values("Drag_Epoch_s")

        drag_plot_time_days = (
            df_drag_plot["Drag_Epoch_s"].values
            - df_drag_plot["Drag_Epoch_s"].values[0]
        ...
        ax.plot(
            drag_plot_time_days,
            df_drag_plot["Estimated_Cd"].values,
            marker="o",
            linestyle="-."
        )

        ax.set_xlabel("Time since first drag estimate [days]")
        ax.set_ylabel("Estimated drag coefficient Cd [-]")
        ax.set_title("Estimated drag coefficient over time")
        ax.grid()

        fig.savefig(
            results_folder / "drag_coefficient_estimates_over_time.png",
            dpi=300,
            bbox_inches="tight"
        )
    ...
    if len(df_biases) > 0:
        ...
        df_bias_plot = df_biases.copy()

        df_bias_plot = df_bias_plot.dropna(
            subset=["Bias_Epoch_s"]
        )

        df_bias_plot = df_bias_plot.sort_values("Bias_Epoch_s")

        bias_plot_time_days = (
            df_bias_plot["Bias_Epoch_s"].values
            - df_bias_plot["Bias_Epoch_s"].values[0]
        ) / constants.JULIAN_DAY

        # Constant bias plot
        if "Absolute_Constant_Bias" in df_bias_plot.columns:
            ...
            ax.plot(
                bias_plot_time_days,
                df_bias_plot["Absolute_Constant_Bias"].values,
                marker="o",
                linestyle="-."
            )

            ax.set_xlabel("Time since first bias estimate [days]")
            ax.set_ylabel("Absolute constant bias")
            ax.set_title("Absolute constant bias over time")
            ax.grid()

            fig.savefig(
                results_folder / "absolute_constant_bias_over_time.png",
                dpi=300,
                bbox_inches="tight"
            )
        ...
        # Linear bias plot
        if "Linear_Constant_Bias" in df_bias_plot.columns:
            ...
            ax.plot(
                bias_plot_time_days,
                df_bias_plot["Linear_Constant_Bias"].values,
                marker="o",
                linestyle="-."
            )

            ax.set_xlabel("Time since first bias estimate [days]")
            ax.set_ylabel("Linear absolute bias")
            ax.set_title("Linear absolute bias over time")
            ax.grid()

            fig.savefig(
                results_folder / "linear_absolute_bias_over_time.png",
                dpi=300,
                bbox_inches="tight"
            )
